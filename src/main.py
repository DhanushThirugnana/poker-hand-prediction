...
from utils import remove_least_quality_rdd, get_k_rdds_from_list, get_merged_rdd
...
def submit_spark_app(host, port):
    ...
    preprocessed_batch_of_poker_hands_rdd = batch_of_poker_hands_dstream.map(tokenize).filter(not_contains_null).map(
        make_labeled_point)
...
def process_rdd(train_rdd):
    ...
    if len(RDD_LIST) > MINIMUM_RDD_LIST_LEN:
        remove_least_quality_rdd(RDD_LIST, rddlist_accuracy_rdd, minimum_list_len=MINIMUM_RDD_LIST_LEN,
                                 optimizing_fn=max)
